...
public class Attack : MonoBehaviour
{
    ...
    IEnumerator AttackSequence(GameObject target, int score)
    {
        ...
        if (diceType != null && role != null && role.DiceObjects != null && target != null)
        {
            ...
            for (int i = 0; i < role.diceCount; i++)
            {
                ...
                {
                    // 비시너지 → 주사위 눈 갯수만큼 bullet 순차 발사
                    anyFired = true;
                    int faceValue = role.GetDiceValue(i);
                    ...
                    for (int d = 0; d < faceValue; d++)
                    {
                        ...
                        if (d == 0)
                            ...
                            StartCoroutine(DelayedFireVfx(data.bulletPrefab, origin, targetPos, travelTime, d * dotFireDelay, spawned));
                    }
                }
            }
    ...
    IEnumerator DelayedFireVfx(GameObject prefab, Vector3 origin, Vector3 targetPos, float duration, float delay, List<GameObject> spawned)
    {
        yield return new WaitForSeconds(delay);
        FireVfx(prefab, origin, targetPos, duration, spawned);
    }
